;(function ($, window, document, undefined) {
    ...
    var VariationForm = function ($form) {
        ...
        self.$customButtons = $form.find('.custom-color-button, .custom-button, .custom-image-button');
        ...
        // FIXED: Listen to changes on select elements, not buttons
        ...
        // Custom button click event - FIXED
        self.$customButtons.on('click', function (e) {
            e.preventDefault();
            var $button = jQuery(this);

            // Don't allow clicking disabled buttons
            if ($button.hasClass('disabled-option')) {
                return;
            }
            ...
            // Remove "no matching variations" message before changing
            self.$form.find('.wc-no-matching-variations').parent().remove();

            // Check if this button is already active
            var isActive = $button.hasClass('active');

            // Find siblings in the same attribute group
            var $siblings = self.$form.find('[data-variation-name="' + attributeName + '"]');
            ...
            if (isActive) {
                ...
                // If clicking an inactive button, select it and deselect siblings
    ...
    VariationForm.prototype.onReset = function (event) {
        ...
        // Remove all tooltips
        form.$form.find('.stock-tooltip-varimo-swatches').remove();

        // Reset select fields
        form.$attributeFields.val('').trigger('change');

        // Reset custom buttons
        form.$customButtons.removeClass('active');

        form.$form.trigger('announce_reset');
        form.$form.trigger('reset_data');
        form.$form.trigger('reset_focus');
    ...
    VariationForm.prototype.onResetVariationFocus = function( event ) {
        // Focus first visible button or select
        var $firstButton = event.data.variationForm.$customButtons.first();
        if ($firstButton.length) {
            $firstButton.focus();
        } else if (event.data.variationForm.$attributeFields.length) {
    ...
    VariationForm.prototype.onUpdateAttributes = function (event) {
        ...
        form.$attributeFields.each(function() {
            ...
            var attrName = $field.data('attribute_name') || $field.attr('name');
            selectedValues[attrName] = $field.val();
        });
        ...
        form.$attributeFields.each(function (index, el) {
            ...
            // Collect all available values for this attribute based on current selections
            var availableValues = [];
            variations.forEach(function(variation) {
                var attrValue = variation.attributes[attributeName];
                if (attrValue && availableValues.indexOf(attrValue) === -1) {
                    // Check if variation is available based on settings
                    if (varimo_variation_swatches.variationSwatchesDisableSettings === "not-disable" ||
                        (variation.is_in_stock && variation.is_purchasable)) {
                        availableValues.push(attrValue);
                    }
                } else if (attrValue === '' || attrValue === undefined) {
                    // "Any" attribute - all values are available
                }
            });
            ...
            $select.find('option').each(function() {
                ...
                var isAvailable = availableValues.indexOf(optionValue) !== -1 ||
                    variations.some(function(v) {
                        return v.attributes[attributeName] === '' || v.attributes[attributeName] === undefined;
                    });
            ...
            // Process custom buttons for this attribute
            form.$customButtons.each(function () {
                var $button = jQuery(this);
                if ($button.data('variation-name') === attributeName) {
                    var buttonValue = String($button.data('value') || '');

                    // Check if this button value is available
                    var isEnabled = availableValues.indexOf(buttonValue) !== -1 ||
                    ...
                    // Update button state
                    if (!isEnabled) {
                        $button.addClass('disabled-option').css('opacity', '0.5');
                        // If this button is currently active but no longer available, remove active class
                        if ($button.hasClass('active')) {
                            $button.removeClass('active');
                            $select.val('').trigger('change');
                        }
                    } else {
                        $button.removeClass('disabled-option').css('opacity', '1');
                    }

                    // Check if other attributes are selected for stock tooltip
                    ...
                    if (otherAttributesSelected && varimo_variation_swatches.variationStockInfo === 'true' && isEnabled) {
                        // Find matching variations for current selections + this button's value
                        var matchingVariations = form.variationData.filter(function(variation) {
    ...
    VariationForm.prototype.getChosenAttributes = function () {
        ...
        this.$attributeFields.each(function () {
            var attributeName = jQuery(this).data('attribute_name') || jQuery(this).attr('name');
